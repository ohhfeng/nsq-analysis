...
// Channel 是 nsqd 内非常重要的数据类型，每个 topic 可以拥有多个 Channel
...
// 初始化 deferredMessages，deferredPQ，inFlightMessages，inFlightPQ
...
// Exiting 返回当前 channle 是否处于 close/delete 状态
...
func (c *Channel) exit(deleted bool) error {
	...
	if !atomic.CompareAndSwapInt32(&c.exitFlag, 0, 1) { // 首先判断当前 channle 是否处于 close/delete 状态
	...
	if deleted {
		...
		c.ctx.nsqd.Notify(c) // 如果是删除的话，通知 lookupdLoop，执行 UNREGISTER 告知 nsqlookupd
	} else {
	...
	if deleted {
		c.Empty()                 // 清空 message chan 以及二级存储队列
		return c.backend.Delete() // 删除二级存储队列
	}

	c.flush()                // 将 message chan，deferredPQ，inFlightPQ 写入二级存储队列
	return c.backend.Close() // 关闭二级存储队列
}

// Empty 清空内存队列以及二级存储队列
...
// 将 message chan，deferredPQ，inFlightPQ 写入二级存储队列
...
// TouchMessage 重置 in_flight message 的过期时间
